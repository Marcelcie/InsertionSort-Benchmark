...
Stats insertionSortClassic(int T[], int n) {
    ...
    for (int i = 1; i < n; i++) {
        ...
        if (j >= 0) s.porownania++; // ostatnie porównanie zakończone fałszem
        T[j + 1] = x;
    }
...
int main() {
    ...
    if (rodzaj_sortowania < 0 || rodzaj_sortowania > 2) {
        ...
        for (int i = 0; i < 100; i++) {
                // 1. Tworzymy wektor z danymi wejściowymi (oryginał dla tej iteracji)
                vector<int> original(n);
                generator[rodzaj_sortowania](original.data(), n);

                // 2. Kopiujemy oryginał do wektora roboczego i sortujemy (Klasyczne)
                working = original;
                ...
                // 5. To samo dla Liniowego Lustra
                working = original;
